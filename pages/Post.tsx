      ...
      <div className="bg-stone-50 min-h-screen pb-24">
        <header className="pt-32 pb-16 bg-white border-b border-stone-100">
            <div className="container mx-auto px-4 md:px-8 max-w-7xl text-center">
                ...
                <h1 className="font-serif text-3xl md:text-4xl lg:text-5xl text-stone-900 leading-[1.1] mb-8 max-w-3xl mx-auto">{post.title}</h1>
                {post.custom_excerpt && <p className="font-serif text-lg md:text-xl text-stone-500 font-light leading-relaxed max-w-2xl mx-auto">{post.custom_excerpt}</p>}
            </div>
            <div className="container mx-auto px-4 md:px-8 max-w-7xl mt-12">
                ...
                <div className="flex justify-between items-start mt-4 px-1 relative">
                    ...
                    <div className="relative" ref={shareRef}>
                        <button onClick={() => setIsShareOpen(!isShareOpen)} className="flex items-center gap-2 text-xs uppercase font-bold tracking-widest text-stone-500 hover:text-stone-900 transition-colors">
                        ...
                        {isShareOpen && (
                            <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-stone-100 shadow-xl rounded-sm z-50 animate-fade-in-up origin-top-right">
                                <div className="p-1">
                                    <button onClick={() => handleShare('facebook')} className="w-full flex items-center gap-3 px-4 py-3 text-xs font-bold text-stone-600 hover:bg-stone-50 hover:text-stone-900 transition-colors text-left"><Facebook size={14} /> Facebook</button>
                                    <button onClick={() => handleShare('twitter')} className="w-full flex items-center gap-3 px-4 py-3 text-xs font-bold text-stone-600 hover:bg-stone-50 hover:text-stone-900 transition-colors text-left"><Twitter size={14} /> Twitter / X</button>
                                    <button onClick={() => handleShare('copy')} className="w-full flex items-center gap-3 px-4 py-3 text-xs font-bold text-stone-600 hover:bg-stone-50 hover:text-stone-900 transition-colors text-left border-t border-stone-100">{copied ? <Check size={14} className="text-green-600" /> : <LinkIcon size={14} />}{copied ? 'Copied!' : 'Copy Link'}</button>
                                </div>
                            </div>
                        )}
                    </div>
        ...
        <div className="container mx-auto px-4 md:px-8 max-w-7xl mt-16 mb-24">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
                <aside className="hidden lg:block lg:col-span-2 sticky top-32 order-1">
                    {toc.length > 0 && (
                        <div>
                             <span className="text-xs font-bold tracking-[0.2em] uppercase text-stone-400 mb-6 block">Contents</span>
                            <nav className="relative border-l border-stone-200">
                                ...
                                {toc.map((item) => {
                                    ...
                                    return (
                                        ...
                                        <a href={`#${item.id}`} className={`block py-2 pl-4 text-sm leading-relaxed transition-all duration-300 border-l-2 -ml-[1px] ${item.level === 3 ? 'pl-8 text-xs opacity-90' : ''} ${isActive ? 'border-stone-900 text-stone-900 font-bold' : 'border-transparent text-stone-400 hover:text-stone-600 hover:border-stone-300'}`}
                                            onClick={(e) => {
                                            e.preventDefault();
                    ...
                    {post.tags && post.tags.length > 0 && (
                        <div className="mt-12">
                            <span className="text-xs font-bold tracking-[0.2em] uppercase text-stone-400 mb-4 block">Tags</span>
                            <div className="flex flex-wrap gap-2">
                                {post.tags.map(tag => (
                                    <Link key={tag.id} to={`/tag/${tag.slug}`} className="text-[10px] uppercase tracking-wide font-bold text-stone-400 hover:text-stone-900 transition-colors">#{tag.name}</Link>
                                ))}
                            </div>
                        </div>
                    )}
                ...
                <main className="col-span-1 lg:col-span-8 order-2 min-w-0 bg-white p-8 md:p-14 shadow-sm rounded-sm">
                    ...
                    <div className="lg:hidden mb-8 border-b border-t border-stone-100 py-4">
                        ...
                        <span className="text-sm font-bold tracking-widest uppercase text-stone-600 flex items-center"><List size={16} className="mr-3" /> Table of Contents</span>
                    ...
                    <div className="mb-8 text-center"><p className="text-[10px] text-stone-300 uppercase tracking-widest font-medium">Contains affiliate links</p></div>
                    <div className="gh-content max-w-none prose prose-stone prose-lg mx-auto prose-headings:font-serif prose-headings:font-normal prose-img:rounded-sm prose-img:w-full prose-a:text-stone-900 hover:prose-a:text-stone-600" dangerouslySetInnerHTML={{ __html: processedHtml || post.html }} />
                    <div className="flex items-center justify-center mt-20 mb-16 opacity-20"><div className="w-16 h-px bg-stone-900"></div></div>
                    {post.primary_author && (
                        <div className="flex items-center gap-6 pt-8 border-t border-stone-100">
                            <img src={post.primary_author.profile_image || "https://picsum.photos/100/100"} alt={post.primary_author.name} className="w-16 h-16 rounded-full grayscale object-cover border border-stone-100 shrink-0" loading="lazy" />
                            <div>
                 ...
                 <aside className="hidden lg:block lg:col-span-2 sticky top-32 order-3">
                    <div>
                        <span className="text-xs font-bold tracking-[0.2em] uppercase text-stone-400 mb-6 block">Trending</span>
                        <div className="space-y-10">
                            {trendingPosts.map((trend, idx) => (
                                <Link to={`/${trend.slug}`} key={trend.id} className="group block">
                                    ...
                                    <div><h4 className="font-serif text-base text-stone-900 leading-tight group-hover:text-stone-600 transition-colors font-bold">{trend.title}</h4></div>
                                </Link>
                            ))}
                        </div>
                    </div>
        ...
        <section className="bg-stone-900 text-stone-100 py-20 mb-20">
            <div className="container mx-auto px-4 text-center max-w-2xl">
                ...
                <button onClick={() => openNewsletter()} className="inline-flex items-center gap-3 bg-white text-stone-900 px-8 py-4 text-xs font-bold tracking-widest uppercase hover:bg-stone-200 transition-colors">Subscribe Now <ArrowRight size={14} /></button>
            </div>
        </section>
        {relatedPosts.length > 0 && (
            <section className="container mx-auto px-4 md:px-8 max-w-7xl">
                <div className="flex items-center justify-between mb-12 border-b border-stone-200 pb-4"><span className="text-xs font-bold tracking-[0.2em] uppercase text-stone-400">Read Next</span></div>
