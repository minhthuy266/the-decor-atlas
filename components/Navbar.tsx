...
const Navbar: React.FC = () => {
  ...
  const getLinkClass = (path: string) => {
    ...
    const activeStyle = isActive
      ? isTransparent
        ...
        : "text-stone-900 opacity-100 border-b-2 border-stone-900"
  ...
  // Check if Explore should be active (any tag page)
      ...
      >
        <div className="container mx-auto px-4 md:px-8 max-w-7xl flex justify-between items-center">
          ...
          <Link to="/" className="z-50 relative">
            <h1
              className={`font-serif text-2xl md:text-3xl tracking-tight font-bold transition-colors duration-300 ${logoClass}`}
            >
              The Decor Atlas.
            </h1>
          </Link>
          ...
          <nav className="hidden md:flex items-center space-x-8 text-sm font-semibold tracking-wide transition-colors duration-300">
            ...
            <div className="relative group">
              <button
                className={`flex items-center transition-all duration-300 focus:outline-none pb-1 ${exploreActiveStyle} ${textBase} ${textHover}`}
                ...
                <div className="absolute top-full -left-4 pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform translate-y-2 group-hover:translate-y-0">
                  <div className="bg-white shadow-xl border border-stone-100 rounded-sm w-64 max-h-[60vh] overflow-y-auto no-scrollbar py-2 flex flex-col">
                    {tags.length > 0 ? (
                      ...
                      <span className="px-6 py-3 text-stone-300 text-xs italic">
                        Loading...
                      </span>
                    )}
                  </div>
            ...
            <Link
              to="/shop"
              className={`flex items-center ${getLinkClass("/shop")}`}
            >
              Shop
            </Link>
          ...
          <button
            className={`md:hidden z-50 transition-colors duration-300 ${
              isTransparent ? "text-white drop-shadow-md" : "text-stone-800"
            }`}
          ...
          {/* Mobile Menu Overlay */}
          <div
            className={`fixed inset-0 bg-stone-50 z-40 transition-opacity duration-300 ${
              isOpen
          ...
          >
            <div className="flex flex-col h-full pt-28 px-8 pb-10">
              ...
              <div className="flex-1 overflow-y-auto no-scrollbar flex flex-col items-center space-y-6">
                <Link
                  to="/"
                  className="font-serif text-3xl text-stone-900 hover:text-stone-600 font-bold"
                >
                  Journal
                </Link>
                ...
                <span className="text-xs font-bold tracking-widest uppercase text-stone-400 shrink-0">
                  Collections
                </span>
                ...
                <div className="flex flex-col items-center space-y-4 w-full">
                  {tags.map((tag) => (
                    <Link
                      key={tag.id}
                      to={`/tag/${tag.slug}`}
                      className="font-serif text-2xl text-stone-800 hover:text-stone-500 capitalize text-center font-semibold"
                    >
                      {tag.name}
                    </Link>
                  ))}
                ...
                <Link
                  to="/about"
                  className="font-serif text-3xl text-stone-900 hover:text-stone-600 shrink-0 pb-8 font-bold"
                >
                  About
                </Link>
              </div>
